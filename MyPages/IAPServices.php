...
function IAP_Program_Init() {
	...
	for ($i = 0; $i < count($MyU); $i++) {
		$ln = array_pop($MyU);
		if (strpos(strtolower($ln), "litehausconsulting") !== FALSE
		or  strpos(strtolower($ln), "itsapartydsr.com") !== FALSE
		or  strpos(strtolower($ln), "iapdsr.com") !== FALSE) {
			array_push($MyU, $ln);
			$MyURL = implode("/", $MyU);
			$_REQUEST['LHCUrl'] = $MyURL."/";
			break;
		}
	}
	if ($ln == "itsapartydsr.com"
	or  $ln == "iapdsr.com") {
...
function IAP_Verify_Password($iapPswd) {
	...
	echo "Unfortunately,";
	echo "&nbsp;&nbsp&nbsp;it is necessary to log you off to protect our system and your data.";
	echo "WE ARE EXTREMELY SORRY for the inconvience.";
	echo "<span style='pad_left: 30px;'>- LHC IAP Support";

	$iapSMSMsg = $iapUser."(".$iapComment.")".$iapPage;
...
function IAP_Get_Event_By_Id($iapEventId, $iapGetRepeats = "Y") {
	...
	$iapPass['table'] = "iapcal";
	if ($iapGetRepeats == "Y") {
		$iapPass['join'] = "LEFT JOIN iap_cal_repeating ON iap_cal_repeating.cr_id = iap_calendar.event_id ";
		$iapPass['join'] = $iapPass['join']."LEFT JOIN iap_cal_exceptions ON iap_cal_exceptions.ce_id ";
	}
	$iapPass['where']= "event_account = '".$_REQUEST['CoId']."' AND event_id = '".$iapEventId."'";
	$iapRet = (array) IAP_Get_Rows($iapPass);
	if ($iapRet['retcode'] < 0) {
...
function IAP_Get_CO_Catalog($iapItemNo, $UseDate) {
	...
	$iapPass['join'] = "LEFT JOIN iap_inventory ON iap_inventory.inv_company = iap_catalog.cat_company AND UPPER(iap_inventory.inv_item_code) = UPPER(iap_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." LEFT JOIN iap_prices ON iap_prices.prc_company = iap_catalog.cat_company AND UPPER(iap_prices.prc_item_code) = UPPER(iap_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." LEFT JOIN iap_codes ON iap_codes.code_company = iap_catalog.cat_company AND iap_codes.code_code = iap_prices.prc_cat_code";
...
function IAP_Get_SUPP_Catalog($iapItemNo, $iapPurDate, $iapSource) {
	...
	$iapPass['join'] = "LEFT JOIN iap_inventory ON inv_company = ".$_REQUEST['CoId'] ." AND UPPER(inv_item_code) = UPPER(cat_item_code)";
	$iapPass['join'] = $iapPass['join']." LEFT JOIN iap_supplier_prices ON iap_supplier_prices.prc_supplier_id = iap_supplier_catalog.cat_supplier_id AND UPPER(iap_supplier_prices.prc_item_code) = UPPER(iap_supplier_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." LEFT JOIN iap_supplier_codes ON iap_supplier_codes.code_supplier_id = iap_supplier_catalog.cat_supplier_id AND iap_supplier_codes.code_code = iap_supplier_prices.prc_cat_code";
...
function IAP_Get_SUPP_Only($iapItemNo, $iapSource) {
	...
	$iapPass['join'] = "LEFT JOIN iap_inventory ON iap_inventory.inv_company = ".$_REQUEST['CoId']." AND UPPER(iap_inventory.inv_item_code) = UPPER(iap_supplier_catalog.cat_item_code)";
	if ($iapSource == "") {
		$iapSupps = $_REQUEST['UserData']['Suppliers'];
		$iapWhere = "";
		$o = "";
...
function IAP_Get_CO_Act($iapItemNo) {
	...
	$iapPass['join'] = "LEFT JOIN iap_purchase_detail on purdet_company = cat_company AND UPPER(purdet_item_code) = UPPER(cat_item_code) ";
	$iapPass['join'] = 	$iapPass['join']."LEFT JOIN iap_sales_detail on saledet_company = cat_company AND UPPER(saledet_item_code) = UPPER(cat_item_code) ";	
...
function IAP_Get_SUPP_Act($iapItemNo) {
	...
	$iapPass['join'] = "LEFT JOIN iap_purchase_detail on purdet_company = ".$_REQUEST['CoId']." AND UPPER(purdet_item_code) = UPPER(iap_supplier.catalog.cat_item_code) ";
	$iapPass['join'] = 	$iapPass['join']."LEFT JOIN iap_sales_detail on saledet_company = ".$_REQUEST['CoId']." AND UPPER(saledet_item_code) = UPPER(iap_supplier.catalog.cat_item_code) ";
...
// >>> 4/20/19 Removed code_type from query
	...
	$iapPass['join'] = "LEFT JOIN iap_inventory ON iap_inventory.inv_company = iap_catalog.cat_company AND UPPER(iap_inventory.inv_item_code) = UPPER(iap_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." JOIN iap_prices ON iap_prices.prc_company = iap_catalog.cat_company AND UPPER(iap_prices.prc_item_code) = UPPER(iap_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." JOIN iap_codes ON iap_codes.code_company = iap_catalog.cat_company AND iap_codes.code_code = iap_prices.prc_cat_code";
...
// >>> 4/20/19 removed code_type from query (should not have been there!)
	...
	$iapPass['join'] = "LEFT JOIN iap_inventory ON iap_inventory.inv_company = iap_supplier_catalog.cat_company AND UPPER(iap_inventory.inv_item_code) = UPPER(iap_supplier_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." JOIN iap_supplier_prices ON iap_supplier_prices.prc_company = iap_supplier_catalog.cat_company AND UPPER(iap_supplier_prices.prc_item_code) = UPPER(iap_supplier_catalog.cat_item_code)";
	$iapPass['join'] = $iapPass['join']." JOIN iap_supplier_codes ON iap_supplier_codes.code_company = iap_supplier_catalog.cat_company AND iap_supplier_codes.code_code = iap_supplier_prices.prc_cat_code";
...
function IAP_Get_CO_LowLevel() {
	$iapPass['table'] = "ctlg";
	$iapPass['cols'] = "iap_catalog.cat_item_code, iap_catalog.cat_description, iap_inventory.inv_on_hand, iap_inventory.inv_min_onhand";
	$iapPass['join'] = "LEFT JOIN iap_inventory ON iap_inventory.inv_company = iap_catalog.cat_company AND iap_inventory.inv_item_code = iap_catalog.cat_item_code";
	$iapPass['where'] = "iap_catalog.cat_company=".$_REQUEST['CoId']." AND iap_inventory.inv_min_onhand > 0 AND iap_inventory.inv_on_hand <= iap_inventory.inv_min_onhand";
	$iapPass['order'] = "iap_catalog.cat_item_code";
	return($iapPass);
}
function IAP_Get_SUPP_LowLevel($Supp = "") {
	$iapPass['table'] = "supcat";
	$iapPass['cols'] = "iap_supplier_catalog.cat_item_code, iap_supplier_catalog.cat_description, iap_inventory.inv_on_hand, iap_inventory.inv_min_onhand";
	$iapPass['join'] = "LEFT JOIN iap_inventory ON iap_inventory.inv_company = ".$_REQUEST['CoId']." AND iap_inventory.inv_item_code = iap_supplier_catalog.cat_item_code";
	$iapWhere = "";
	if ($Supp == "") {
		$iapSupps = $_REQUEST['UserData']['Suppliers'];
		$o = "";
		...
		$iapWhere = "iap_supplier_catalog.cat_supplier_id = ".strval($Supp);		
	}
	$iapPass['where'] = "iap_inventory.inv_min_onhand > 0 AND iap_inventory.inv_on_hand <= iap_inventory.inv_min_onhand AND (".$iapWhere.")";
	$iapPass['order'] = "iap_supplier_catalog.cat_item_code";
	return($iapPass);
}
...
function IAP_Build_CoSupp_Array() {
	...
	$iapPass['join'] = "JOIN iap_suppliers ON supp_id = cs_supplier";
...
function IAP_Get_CoUser($cu_id = 0) {
    ...
    $iapPass['join'] = "JOIN iap_company ON co_id = cu_company";
	...
	if ($iapRet['retcode'] < 0) {
		...
		echo "Serv back from get couser get_rows with error...<pre>";;
        var_dump($iapRet);
        echo "</pre>";
...
//	$iapPass['cols'] = "cust_no, cust_name, cust_email, cust_phone";
	...
	$iapPass['join'] = "LEFT JOIN iap_avalara_sales_tax ON tax_zip_code = LEFT(cust_zip, 5)";
    $iapPass['where'] = "cust_company='".$_REQUEST['CoId']. "'";
    if ($iapHow == "E") {
...
function IAP_Get_GiftCert_List() {
    ...
    $iapPass['join'] = "JOIN iap_customers ON cust_no = gc_for_custno";
...
function IAP_Get_PartyEvent_By_Id($iapPEID) {
    ...
    $iapPass['join'] = "LEFT JOIN iap_avalara_sales_tax on iap_avalara_sales_tax.tax_zip_code = iap_party_events.pe_zip ";
...
function IAP_Get_PE_List($peClosed = "Y", $pePartyOnly = "N") {
	...
	$iapPass['join'] = "LEFT JOIN iap_avalara_sales_tax ON tax_zip_code = LEFT(pe_zip, 5)";
...
function IAP_Get_PurDet($iapPurId) {
	...
	$iapPass['join'] = "LEFT JOIN iap_catalog ON iap_catalog.cat_company = iap_purchase_detail.purdet_company AND iap_catalog.cat_item_code = iap_purchase_detail.purdet_item";
	$iapPass['join'] = $iapPass['join']." LEFT JOIN iap_supplier_catalog ON iap_supplier_catalog.cat_supplier_id = iap_purchase_detail.purdet_source AND iap_supplier_catalog.cat_item_code = iap_purchase_detail.purdet_item ";
...
function IAP_Get_PurDet_For_Item($iapItem, $iapFrom) {
	...
	$iapPass['join'] = "JOIN iap_purchases on pur_company = purdet_company AND pur_id = purdet_purid";
...
function IAP_Get_Sale_By_PE($iapPEId) {
	...
	$iapPass['join'] = "LEFT JOIN iap_customers ON iap_customers.cust_company = iap_sales.sale_company AND iap_customers.cust_no = iap_sales.sale_customer ";
...
function IAP_Get_Sale_By_Cust($iapCust) {
	...
	$iapPass['table'] = "sale";
	$iapPass['join'] = "JOIN iap_party_events ON pe_company = sale_company AND pe_id = sale_peid";
...
function IAP_Get_Sale_List() {
	...
	$iapPass['join'] = "JOIN iap_customers ON iap_customers.cust_company = iap_sales.sale_company AND iap_customers.cust_no = iap_sales.sale_customer ";
	$iapPass['join'] = $iapPass['join']."LEFT JOIN iap_party_events ON pe_company = sale_company AND pe_id = sale_peid";
...
function IAP_Get_SaleDet($iapSaleId) {
	...
	$iapPass['join'] = $iapPass['join']."JOIN iap_sales ON sale_company = saledet_company AND sale_id = saledet_sid ";
	$iapPass['join'] = $iapPass['join']."JOIN iap_party_events ON pe_company = saledet_company AND pe_id = sale_peid";
...
function IAP_Get_SaleDet_For_Cust($iapCust) {
	...
	$iapPass['join'] = $iapPass['join']." LEFT JOIN iap_supplier_catalog on iap_supplier_catalog.cat_supplier_id = saledet_item_source AND UPPER(iap_supplier_catalog.cat_item_code) = UPPER(saledet_item_code)";
	$iapPass['join'] = $iapPass['join']." JOIN iap_sales ON sale_company = saledet_company AND sale_id = saledet_sid";
	$iapPass['join'] = $iapPass['join']." JOIN iap_party_events ON pe_company = sale_company AND pe_id = sale_peid";
...
function IAP_Get_CO_SaleDet4Items($iapItem, $iapFrom) {
	...
	$iapPass['join'] = "JOIN iap_catalog on UPPER(cat_item_code) = UPPER(saledet_item_code)";
	$iapPass['join'] = $iapPass['join']." JOIN iap_sales ON sale_company = saledet_company AND sale_id = saledet_sid";
	$iapPass['join'] = $iapPass['join']." JOIN iap_party_events ON pe_company = sale_company AND pe_id = sale_peid";
	$iapPass['join'] = $iapPass['join']." JOIN iap_customers ON cust_company = saledet_company AND cust_no = saledet_customer_no";
...
function IAP_Get_SUPP_SaleDet4Items($iapItem, $iapFrom) {
	...
	$iapPass['join'] = "JOIN iap_supplier_catalog on cat_supplier_id = saledet_item_source ". 
...
echo "LHC Path is ".$_REQUEST['LHCPath']."<br>";
echo "Running app is ".$_REQUEST['runningapp']."<br>";



$MyURLIn = "http://".$_SERVER["HTTP_HOST"].$_SERVER["PHP_SELF"];
if  (stripos($MyURLIn, "itsapartydsr.com") === FALSE) {
	$MyU = explode("/", $MyURLIn);
	for ($i = 0; $i < count($MyU); $i++) {
		$ln = array_pop($MyU);
		if (strpos(strtolower($ln), "litehausconsulting") !== FALSE) {
	...
	$MyURL = "http://ItaAPartyDSR.com";
}
$_REQUEST['LHCUrl'] = $MyURL."/";

echo "LHC URL is ".$_REQUEST['LHCUrl']."<br>";



// Get other service modules
// require_once($_REQUEST['LHCPath']."LHC_Services.php");

echo "Requiring IAPDBServices<br>";
