...
// I'll have to do this for each app that uses the calendar
	...
	if (IAP_Program_Start($eePage, "Y") < 0) {
		return;
	}
	$_REQUEST['sec_acct'] = $_REQUEST['CoId'];
	$_REQUEST['sec_client'] = $_REQUEST['IAPUID'];
...
if (!( $eeEvent )) {
	...
	if ($_REQUEST['action'] == "NEW") {
		...
		$eeEvents = (array) $fun(array("table" => $eeDBCal));
		if ($eeEvents < 0) {
		...
		$es = $eeEvent['status'];
		$fun = $eeApp."_Build_New_Row";
		$eeRepeats = (array) $fun(array("table" => $eeDBCRep));
		if ($eeRepeats < 0) {
	...
	if ($_REQUEST['deleteevent'] == "Delete") {
		...
		$eeRet = IAP_Validate_Date($eeEvent['event_begin'], $_REQUEST['eebegdate'], "N");
		...
		$eeRet = IAP_Validate_Date($eeEvent['event_end'], $_REQUEST['eeenddate'], "N");
		...
		$eeRet = IAP_Validate_Time($eeEvent['event_btime'],$_REQUEST['eebegtime'], "N");
		...
		$eeRet = IAP_Validate_Time($eeEvent['event_etime'],$_REQUEST['eeendtime'], "N");
...
<?php
	...
	echo "<form action='?action=".$eeFormRet."&origaction=".$_REQUEST['origaction']."' method='POST'>";
