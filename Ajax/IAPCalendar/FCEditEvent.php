...
/*
    ...
    if ($eePRet['Error'] == "1") {
        ...
        $BDError = 2;
        $PageError = 1;
	} elseif (strtotime($eeEvent['event_begin']) < strtotime("now")) {
        $BDError = "W";
        $PageError = 1;
	}
...
*/
	...
	if (isset($_REQUEST['eelname'])) {
		$eeEvent['event_loc_name'] = $_REQUEST['eename'];
	}

	if (isset($_REQUEST['eelstrt'])) {
		$eeEvent['event_loc_street'] = $_REQUEST['eestrt'];
	}

	if (isset($_REQUEST['eelcity'])) {
		$eeEvent['event_loc_city'] = $_REQUEST['eecity'];
	}

	if (isset($_REQUEST['eelstate'])
	and $_REQUEST['eelstate'] != "--") {
		$eeEvent['event_loc_state'] = $_REQUEST['eestate'];
	}

	if (isset($_REQUEST['eelzip'])) {
		$eeEvent['event_loc_zip'] = $_REQUEST['eezip'];
	}
...
echo "<span style='font-size: 115%;'><strong><em>Add/Update An Event</em></strong></span></center><br />";
	...
	echo "<form action='".$PHP_SELF."?ret=eeRet' method='POST'>";
	...
	echo "<tr><td colspan='2'>";
		echo "<table>";
//		echo "<tr><td width='100'>Begin Date:</td><td width='150'><script>DateInput('eebegdate', true, 'YYYY-MON-DD', '".$eeEvent['event_begin']."')</script></td>";
		echo "<tr><td width='100'>Begin Date:</td><td width='150'>".date("D M j, Y ", strtotime($eeEvent['event_begin']))."</td>";
		echo "<td width='20'></td>";
...
//		echo "<tr><td width='100'>End Date:</td><td width='150'><script>DateInput('eeenddate', true, 'YYYY-MON-DD', '".$eeEvent['event_end']."')</script></td>";
		echo "<tr><td width='100'>End Date:</td><td width='150'>".date("D M j, Y ", strtotime($eeEvent['event_end']))."</td>";
		echo "<td width='20'></td>";
	...
	echo "<tr><td colspan='2'>";	
		echo "<table><tr>";
		echo "<td width='20'></td><td width='80'>Name:</td><td width='500'><input id='eelname' name='eelname' type='text' size='50' maxlength='50' value='".$eeEvent['event_loc_name']."' /></td></tr>";
		echo "<td width='20'></td><td width='80'>Street:</td><td width='500'><input id='eelstrt' name='eelstrt' type='text' size='50' maxlength='50' value='".$eeEvent['event_loc_street']."' /></td></tr>";
		echo "</table>";
		echo "<table><tr>";
...
// Building options for recurring terms
	...
	$RecDayWk = date("w", $eeEvent['event_begin']);
	$RecDayMo = date("j", $eeEvent['event_begin']);
	$RecDayYr = date("z", $eeEvent['event_begin']);
...
// Daily options
		if ($eeEvent['event_recur_period'] == "D") {
			$d = "block ";
		//	echo " checked";
		} else {
			$d = "none ";
...
// Weekly options
		if ($eeEvent['event_recur_period'] == "W") {
			$w = "block ";
		//	echo " checked";
		} else {
			$w = "none ";
...
// Monthly options
		if ($eeEvent['event_recur_period'] == "M") {
			$m = "block ";
		//	echo " checked";
		} else {
			$m = "none ";
...
// Annual options
		if ($eeEvent['event_recur_period'] == "A") {
			$a = "block ";
		//	echo " checked";
		} else {
			$a = "none ";
...
// End of occurs options
		if ($eeEvent['event_recur_period'] == "A" 
		or  $eeEvent['event_recur_period'] == "D" 
		or  $eeEvent['event_recur_period'] == "M"
		or  $eeEvent['event_recur_period'] == "W") {
			$e = "block ";
		//	echo " checked";
		} else {
			$e = "none ";
		...
		echo "<div id='eeendoptions' style='display:".$e."'>";
			...
			echo "<table><tr><td><input type='radio' name='eerecend' value='a' />After </td>".
				 "<td> <script>DateInput('enddate', true, 'YYYY-MON-DD', '".$eeEvent['event_begin']."')</script></td></tr></table><br /><br />";
		echo "</div>";
		echo "</td></tr></table>";
