...
// TODO When an item is added check if it exists in iap_catalog for any user with supplier code 1
...
function StripHdr($SHFile) {
	...
	$HdrEnd = strpos($SHFile, "<!-- Content Here //-->");
...
// put into debugging table
/*  -------------------------------------------------------------------------------------------------
			$s = 'insert into magrun (mr_item, mr_desc) values ("'.$GIICode.'", "'.$GIIDesc.'")';
			require_once($iapPath."/IAPDBServices.php");
			$iapRet = iapProcessMySQL("insert", $s);
			if ($iapRet < 0) {
				echo "<span class=iapError>IAP INTERNAL ERROR inserting item ".$GIICode." into magrun [FATAL]<br>Please notify Support and provide this reference of /".basename(__FILE__)."/".__LINE__."</span><br>";
				exit;
			}
			if ($iapRet['numrows'] == 0){
				echo "<span class=iapError>IAP INTERNAL ERROR item ".$GIICode." not inserted into magrun [FATAL]<br>Please notify Support and provide this reference of /".basename(__FILE__)."/".__LINE__."</span><br>";
				exit;
			}
-------------------------------------------------------------------------------------------------  */
...
cat007	Bittie Bracelets
cat008	Bittie Pendants
cat009	Bittie Pins and Brooches
cat010	Bittie Premium Inserts
cat011	Bittie Standard Inserts
